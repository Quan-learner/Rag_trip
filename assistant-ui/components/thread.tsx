...
const ThreadWelcome: FC = () => {
  return (
    <div className="aui-thread-welcome-root my-auto flex grow flex-col">
      <div className="aui-thread-welcome-center flex w-full grow flex-col items-center justify-center gap-5">
        <div className="aui-thread-welcome-message flex size-full flex-col items-center justify-center px-4 text-center">
          <h1 className="aui-thread-welcome-message-inner fade-in slide-in-from-bottom-1 animate-in fill-mode-both text-2xl font-normal text-zinc-950 dark:text-zinc-100 duration-200">
            开始规划你的下一次旅行
          </h1>
          <p className="aui-thread-welcome-message-inner fade-in slide-in-from-bottom-1 animate-in fill-mode-both inline-block max-w-[34rem] text-left indent-[2em] text-muted-foreground text-xl font-normal delay-75 duration-200">
            告诉我目的地、天数和预算，我会给出建议。
          </p>
        </div>
        <div className="w-full max-w-(--thread-max-width) px-2">
          <Composer />
        </div>
      </div>
      <ThreadSuggestions />
      ...
      <AuiIf condition={(s) => !s.thread.isRunning}>
        <ComposerPrimitive.Send asChild>
          <TooltipIconButton
            ...
            variant="default"
            size="icon"
            className="aui-composer-send size-8 rounded-full bg-black text-white hover:bg-zinc-800 dark:bg-black dark:text-white dark:hover:bg-zinc-800"
...
const ThinkingIndicator: FC = () => {
  return (
    <div
      ...
      className="inline-flex items-center gap-2 rounded-full border border-zinc-800/25 bg-gradient-to-r from-zinc-900/15 via-zinc-700/10 to-zinc-900/15 px-3 py-1 text-xs dark:border-zinc-200/20"
      aria-label="Assistant is working"
    >
      <span className="relative inline-flex size-4 items-center justify-center">
        <span className="absolute size-4 animate-spin rounded-full border-2 border-zinc-900/25 border-t-zinc-900 dark:border-zinc-100/20 dark:border-t-zinc-100" />
        <span className="absolute size-1.5 animate-pulse rounded-full bg-zinc-900/80 dark:bg-zinc-100/80" />
        <SparklesIcon className="relative size-2.5 text-zinc-700 dark:text-zinc-200/90" />
      </span>
      <span className="text-foreground/90 font-medium">思考中</span>
      <span className="inline-flex items-center gap-1">
        <span className="size-1.5 animate-bounce rounded-full bg-zinc-700 dark:bg-zinc-200 [animation-delay:-0.2s]" />
        <span className="size-1.5 animate-bounce rounded-full bg-zinc-700 dark:bg-zinc-200 [animation-delay:-0.1s]" />
        <span className="size-1.5 animate-bounce rounded-full bg-zinc-700 dark:bg-zinc-200" />
      </span>
    </div>
  );
};
